...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'product.create': {
    ...
    key: 'product-added',
    tenantPath: ['marketplace', 'product'],
    userPath: [],
  },
  'product.update': {
    roles: ['business-owner'],
  ...
  'purchase.create': {
    roles: ['business-owner'],
    key: 'new-purchase',
    tenantPath: ['marketplace', 'product', 'purchase'],
    userPath: [],
  },
  'review.create': {
    roles: ['business-owner', 'seller'],
    key: 'new-review',
    tenantPath: ['marketplace', 'product', 'review'],
    userPath: [],
  },
};
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.marketplace.findFirst({
