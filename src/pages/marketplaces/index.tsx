...
type ColumnType = ColumnDef<MarketplaceInterface, unknown>;

interface MarketplaceListPageProps {
...
export function MarketplaceListPage(props: MarketplaceListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MarketplaceInterface>>(
    () => `/marketplaces?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: MarketplaceInterface) => {
    if (hasAccess('marketplace', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/marketplaces/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/marketplaces/view/${record.id}`} passHref legacyBehavior>
                <Button
